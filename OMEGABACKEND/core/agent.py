...
# execute_tool is injected via constructor or lazy-imported at call site
# to break circular dependency: agent.py ↔ tools.py
...
class Agent:
    def __init__(self, config: Config, execute_tool_fn=None) -> None:
        self.config = config
        # Tool execution function — injected to break circular import
        ...
        # Inject tool executor (breaks circular import: agent.py ↔ tools.py)
        self._execute_tool = execute_tool_fn
    ...
    def _call_tool(self, name: str, args: dict) -> Any:
        """Execute a tool by name with lazy import fallback.

        Primary: injected executor (from constructor, breaks circular import).
        Fallback: lazy import from tools module for backward compatibility.
        """
        if self._execute_tool is not None:
            return self._execute_tool(name, args)
        from tools import execute_tool as _et  # noqa: F811
        return _et(name, args)
        ...
        while steps < max_steps:
            ...
            try:
                ...
                for tc in collected_tool_calls:
                    ...
                    result = self._call_tool(tc_name, tc_args)
                    clear_current_tool()
